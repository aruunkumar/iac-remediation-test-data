...
# AGENT-FIXED: CKV_AWS_237 - Added lifecycle block with create_before_destroy to ensure zero-downtime updates
# Create API Gateway
...
# AGENT-FIXED: CKV2_AWS_51 - Created client certificate for mutual TLS authentication between API Gateway and backend
...
# AGENT-FIXED: CKV_AWS_73 - Enabled X-Ray tracing for distributed request tracking and monitoring
# AGENT-FIXED: CKV2_AWS_51 - Added client certificate authentication for secure backend communication
# TODO: CKV_AWS_76 - API Gateway Access Logging not enabled
# Resource: aws_api_gateway_stage.prod
# Reason: Access logging requires a CloudWatch Log Group ARN and log format configuration which are organization-specific
# Fix: To enable access logging:
#   1. Create or identify a CloudWatch Log Group for API Gateway logs
#   2. Add an access_log_settings block to this stage resource with:
#      - destination_arn: ARN of the CloudWatch Log Group (must have proper permissions)
#      - format: Log format string (e.g., JSON format with $context variables)
#   3. Example:
#      access_log_settings {
...
#        })
#      }
# TODO: CKV2_AWS_4 - API Gateway logging level not defined
# Resource: aws_api_gateway_stage.prod
# Reason: Logging level requires creating an aws_api_gateway_method_settings resource and coordination with CloudWatch Logs setup
# Fix: To configure logging level:
#   1. First, ensure access logging is configured (see CKV_AWS_76 above)
#   2. Create an aws_api_gateway_method_settings resource:
...
#        }
#      }
#   3. Ensure the API Gateway service has permission to write to CloudWatch Logs
# TODO: CKV2_AWS_29 - API Gateway not protected by WAF
# Resource: aws_api_gateway_stage.prod
# Reason: WAF configuration requires organization-specific security rules, rate limits, and threat protection policies
...
#        scope = "REGIONAL"
#        default_action {
#          allow {}  # or block {} depending on your security posture
#        }
#        # Add rules as needed (rate limiting, geo blocking, SQL injection protection, etc.)
#        rule {
#          name     = "RateLimitRule"
#          priority = 1
#          statement {
...
#      }
#   3. Configure appropriate WAF rules based on your security requirements
