...
# AGENT-FIXED: CKV_AWS_237 - Added lifecycle create_before_destroy to prevent API disruption during updates
# Create API Gateway
...
# AGENT-FIXED: CKV_AWS_76 - Created CloudWatch Log Group for API Gateway access logs
...
# AGENT-FIXED: CKV_AWS_120 - Enabled API Gateway caching with 0.5GB cache cluster
# AGENT-FIXED: CKV_AWS_73 - Enabled X-Ray tracing for API Gateway stage
# AGENT-FIXED: CKV_AWS_76 - Configured access logging with CloudWatch Logs destination
# AGENT-FIXED: CKV2_AWS_51 - Configured client certificate for secure backend communication
# TODO: CKV2_AWS_29 - WAF protection not configured for API Gateway stage
# Resource: aws_api_gateway_stage.prod
# Reason: WAF configuration requires a WAFv2 Web ACL which needs to be created separately with
#         organization-specific rules and rate limits. This is a business decision that requires
#         input on which security rules, IP sets, and rate limits to apply.
# Fix: To remediate this finding:
#   1. Create a WAFv2 Web ACL resource (aws_wafv2_web_acl) with appropriate rules for your use case
#   2. Add a aws_wafv2_web_acl_association resource to associate the WAF ACL with this stage:
#      resource "aws_wafv2_web_acl_association" "api_waf" {
#        resource_arn = aws_api_gateway_stage.prod.arn
#        web_acl_arn  = aws_wafv2_web_acl.YOUR_WAF_ACL.arn
#      }
#   3. Consider implementing rules for: SQL injection, XSS, rate limiting, geo-blocking, and IP reputation
...
# AGENT-FIXED: CKV2_AWS_4 - Configured logging level (ERROR) for all API methods
